...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(outputs, [p.join(root.path, 'integration_test/a_test.dart')]);
      final first = File(outputs.single).readAsStringSync();
      expect(first, contains('../gherkin/support/patrol_bootstrap.dart'));
      expect(
        Directory(p.join(root.path, 'gherkin/generated')).existsSync(),
        isFalse,
      );
  ...
  test(
    'nested apps generate under their own integration_test directory',
    () async {
      ...
      final nested = ProjectConfig(root.path, {
        ...config.data,
        'app_root': 'apps/mobile',
      });
      configurePatrolDirectory(nested);
      final pub =
          loadYaml(File(p.join(app.path, 'pubspec.yaml')).readAsStringSync())
              as Map;
      expect(pub['patrol']['test_directory'], 'integration_test');
      expect(pub['patrol']['app_name'], 'Mobile');
      final feature = File(
        p.join(root.path, 'gherkin/features/account/login.feature'),
      );
      feature.parent.createSync(recursive: true);
      feature.writeAsStringSync(
        'Feature: Login\n Scenario: S\n  When I wait 1 milliseconds',
      );
      final outputs = await generateSuite(nested, [feature]);
      expect(outputs, [
        p.join(app.path, 'integration_test/account/login_test.dart'),
      ]);
      expect(
        File(outputs.single).readAsStringSync(),
        contains('../../../../gherkin/support/patrol_bootstrap.dart'),
      );
      await generateSuite(nested, [feature], check: true);
      expect(
        Directory(p.join(root.path, 'integration_test/account')).existsSync(),
        isFalse,
      );
    },
  );
  test(
    'Patrol directory setup preserves settings and rejects conflicting suites',
    () {
      final pubspec = File(p.join(root.path, 'pubspec.yaml'));
      configurePatrolDirectory(config);
      final before = pubspec.readAsStringSync();
      expect(
        (loadYaml(before) as Map)['patrol']['test_directory'],
        'integration_test',
      );
      configurePatrolDirectory(config);
      expect(pubspec.readAsStringSync(), before);
      final conflicting = before.replaceAll('integration_test', 'custom_tests');
      pubspec.writeAsStringSync(conflicting);
      expect(
        () => configurePatrolDirectory(config),
        throwsA(isA<GherkinException>()),
      );
      expect(pubspec.readAsStringSync(), conflicting);
    },
  );
  test(
    'generation and stale checking preserve hand-written integration tests',
    () async {
      final feature = File(p.join(root.path, 'gherkin/features/a.feature'))
        ..writeAsStringSync(
          'Feature: A\n Scenario: S\n  When I wait 1 milliseconds',
        );
      final handwritten = File(
        p.join(root.path, 'integration_test/a_test.dart'),
      )..writeAsStringSync('// Hand-written test\nvoid main() {}\n');
      final before = handwritten.readAsStringSync();
      await expectLater(
        generateSuite(config, [feature]),
        throwsA(isA<GherkinException>()),
      );
      expect(handwritten.readAsStringSync(), before);
      handwritten.renameSync(
        p.join(root.path, 'integration_test/smoke_test.dart'),
      );
      await generateSuite(config, [feature]);
      await generateSuite(config, [feature], check: true);
      expect(
        File(
          p.join(root.path, 'integration_test/smoke_test.dart'),
        ).readAsStringSync(),
  ...
  test('full-suite stale check rejects orphaned generated tests', () async {
    ...
    await generateSuite(config, [f]);
    File(
      p.join(root.path, 'integration_test/orphan_test.dart'),
